...
现在完成的是 **Phase 0–1**：把数据格式定下来，并做出一张可校验、可通关的样例地图——**小学一年级「数与运算」**。
...
| 1 | 小学数学样例：一年级数与运算可通关地图 + 1–6 年级标题路书 | ✅ 本仓库 |
...
maps/primary-math/grade-1-numbers/   一年级样例地图
  nodes.json                     知识点
  edges.json                     关系（含前置 DAG）
  quests.json                    任务（每节点至少一个 + 三名关主）
  map.meta.json                  地图标题、起点、通关条件
  player-progress.sample.json    一份演示存档
...
- 给学生看的字用**中文**
- 程序 id 用英文短横线，如 `pm-g1-n001`（primary math，一年级，第 1 号节点）
...
6. 用下面的命令确认这张图没写坏（唯一 id、边都指向存在的点、前置无环、非起点都能从起点走到、每点至少一题）
...
重新生成一年级样例 JSON（改 `scripts/_build_grade1_numbers.py` 之后）：
...
小学数学 1–6 年级领域标题已经列在 [maps/primary-math/OVERVIEW.md](maps/primary-math/OVERVIEW.md)，目前除一年级数与运算外都是**只占题目**。  
