...
| 4 | 游戏客户端或 Agent Skill：读 JSON、改存档、出题 | ✅ 最小可通关网页（`web/`，进度存在浏览器 localStorage；还没有账号 / 服务端 / 真 AI 出题） |

**Phase 1–2 做小学数学，2b 做初中数学，2c 做高中数学核心。** 其它学科先在模式里留好 `subject` 枚举。小 / 初 / 高中数学地图 **schema 仍共用**，只是 `stage`、`grade` 和节点 id 前缀不同。

## 网页怎么玩（最小通关 UI）

仓库里的地图 JSON 已经可以在浏览器里点亮、打关、通关。进度保存在 **本机浏览器**（`localStorage`），没有账号，也没有服务器。
...
玩法：首页按 **小学 / 初中 / 高中** 浏览地图（标题来自各图的 `map.meta.json`，目前约 47 张）→ 点进一张图看节点（锁定 / 可学 / 学习中 / 已点亮）→ 打开节点完成任务 → 灯亮后解锁后续节点 → 击败关主 → 整图通关。可随时「重置本图进度」。

## 目录

```
web/                                 最小可通关网页（Vite + React + TypeScript）
...
最快的方式：按上面「网页怎么玩」启动 `web/`，在浏览器里点亮。规则与 JSON 一致，小学二十四张图、初中十二张图、高中十一张图各自通关，**还没有**跨地图进度引擎（四大领域图也不互相前置；初中图不引用 `pm-*`；高中图不引用 `pm-*` / `jm-*`，高中模块之间也不互写前置）。没有网页时，仍可按下述步骤只读数据走一遍。
...
- 网页是本地通关 UI：任务靠学生按提示自己做，再点「我完成了」；还没有自动出题 / AI 辅导
- 练习题只有提示语，没有自动出题器
- 小学、初中、高中数学核心模块已齐；高中选修（如不等式选讲、空间向量深化、正态计算）以及其他学科地图尚未开始
- `strand` 枚举目前按数学四大领域；其他学科需要扩展字段

这些不挡 Phase 1–2c：模式、47 张样例 DAG、校验、许可证、最小网页已经齐。下一阶段可以是账号存档或 Agent Skill，或其他学科地图。
