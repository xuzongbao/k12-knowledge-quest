...
- `pm` = primary math（小学数学）；`jm` = junior math（初中数学）；`sm` = senior math（高中数学）
- `g1` = 一年级（小学 1–6；初中 7–9；高中节点用模块短码而非强制 g10 文件夹）
- 小学：`n001` / `geo-n001` / `stat-n001` / `prac-n001` = 领域内节点序号
- 初中：`alg-n001` / `geo-n001` / `stat-n001` / `prac-n001`
- 高中：`sm-set-n001` / `sm-func-n001` / `sm-trig-n001` 等（模块短码 + 序号）；关主如 `sm-func-boss-exp`

例：`pm-g1-n001`（小学数与运算）、`jm-g7-alg-n001`（初中七年级数与代数）、`sm-func-n001`（高中函数概念与基本初等函数）。

小学、初中、高中地图共用同一套 schema；`stage` 分别为 `小学` / `初中` / `高中`。高中 `grade` 取 10–12 作为学段带，真实模块名写在地图标题和 `map.meta.json` 的 `module` 字段。
...
Phase 1 的 `strand` 枚举按数学四大领域来（小学、初中、高中地图都复用这四个桶；高中真实模块名写在地图标题和 `module`）。语文、英语等学科扩展时：
