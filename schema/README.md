...
- `pm` = primary math（小学数学）；`jm` = junior math（初中数学）
- `g1` = 一年级（小学 1–6；初中 7–9）
- 小学：`n001` / `geo-n001` / `stat-n001` / `prac-n001` = 领域内节点序号
- 初中：`alg-n001` / `geo-n001` / `stat-n001` / `prac-n001`

例：`pm-g1-n001`（小学数与运算）、`pm-g1-geo-n001`（小学图形与几何）、`jm-g7-alg-n001`（初中七年级数与代数）、`jm-g7-geo-n001`（初中七年级图形与几何）。

小学与初中地图共用同一套 schema；`stage` 分别为 `小学` / `初中`。
